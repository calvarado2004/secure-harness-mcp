...
def repair(a, code, iters, scratch, tools=("bandit",)):
    for _ in range(iters):
        ...
        fix = (f"Here is the current app.py:\n```python\n{code}\n```\n\nProblems to fix:\n"
               + "\n".join(probs)
               + "\n\nFix ONLY these specific problems and any compile error. Do NOT add new "
               "endpoints, features, or functionality, and do NOT expand, refactor, or rewrite "
               "code beyond what these fixes strictly require — keep the application's scope and "
               "behavior identical. Return the complete app.py with only those minimal changes, "
               "in one python block.")
